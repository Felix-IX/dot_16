-- pico ping�(changed the breakout hero)
-- by pico14
...
function _init()
 pirate=false
 ver,url="v1",stat(102)
 if url~="www.lexaloffle.com" and url~="v6p9d9t4.ssl.hwcdn.net" and url~="game.itch" and url~="www.playpico.com" and url~="uploads.ungrounded.net" and url~="game296454.konggames.com" and url~=0 then pirate=true end
...
function restartlevel()
 ...
 ball_dr=0.5
 ball_spd=1
 if fastmode then ball_spd=1.5 end
...
function levelfinished()
 ...
 for i=1,#bricks do
  if bricks[i].v == true and bricks[i].t ~= "i" then return false end
 end
 return true
end
...
function getpoints(_p)
 if fastmode then _p=_p*2 end
 if timer_reduce<=0 then
  points = points + _p*chain*pointsmult
 else
  points = points + (_p*chain*pointsmult)*10
 end
...
function check_sd()
 local c=0
 if sd_brick~=nil then return  end
 for i=1,#bricks do
  if bricks[i].v==true and bricks[i].t ~= "i" then c = c + 1 end
 ...
 for i=1,#bricks do
  ...
  if _b.v or _b.fsh>0 then
   if _b.dx~=0 or _b.dy~=0 or _b.ox~=0 or _b.oy~=0 then
    ...
    _b.dx = _b.dx - _b.ox/10
    _b.dy = _b.dy - _b.oy/10
...
function update_sash()
 if sash_v then
  sash_frames = sash_frames + 1
  if sash_delay_w>0 then
   sash_delay_w = sash_delay_w - 1 
  else
   sash_w = sash_w + (sash_dw-sash_w)/5
  ...
  if sash_delay_t>0 then
   sash_delay_t = sash_delay_t - 1
  else 
   sash_tx = sash_tx + (sash_tdx-sash_tx)/10
...
function update_winnerwait()
 govercountdown = govercountdown - 1
 ...
 if govercountdown<0 then
  if loghs then
   ...
   if btnp(2) then
    if nit_sel<4 then
     sfx(16)
     nitials[nit_sel] = nitials[nit_sel] - 1
...
function update_game()
 ...
 -- powerup timers
 if timer_mega > 0 then
  timer_mega = timer_mega - 1
 end
 if timer_mega_w > 0 then
  timer_mega_w = timer_mega_w - 1
 end 
 if timer_slow > 0 then
  timer_slow = timer_slow - 1
 end
 if timer_expand > 0 then
  timer_expand = timer_expand - 1
 end
 if timer_reduce > 0 then
  timer_reduce = timer_reduce - 1
 end
 animatebricks()
...
function draw_start()
 ...
 palt()
 print("by pico14",25+(hs_x-128),50,2)
 print("",25+(hs_x-128),56,2)
 
 print("music by grubermusic",25+(hs_x-128),64,2)
 print("",25+(hs_x-128),70,2)
 ...
 if hs_x>=0 and not(pirate) then
  ...
  print("press �� to toggle fast mode",4,115,3)
  if hs_x==128 then
   print("press � for high score list",9,109,3)
  end
 end
 
 if pirate then print(stat(102),9,109,1) end
...
function draw_gameover()
 ...
 print("press � or � to retry level",8,68,_c1)
 print("press � or � for main menu",8,74,_c2)
end
...
function draw_levelover()
 ...
 print("press � or � to continue",12,68,blink_b)
end
...
function pointstring(s2,s1)
 if s1==0 and s2==0 then return "0" end
 local ret=""
 if s2>0 then
  ret=ret..s2
  if s1==0 then
